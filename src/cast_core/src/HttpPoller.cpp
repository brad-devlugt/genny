...
// them to the `_loop{}` initializer in the `HttpPoller` constructor
...
void HttpPoller::run() {
    ...
    for (auto&& config : _loop) {
        ...
        for (const auto&& _ : config) {
            ...
            // debug level.
            //
            BOOST_LOG_TRIVIAL(debug) << " HttpPoller Inserting "
...
HttpPoller::HttpPoller(genny::ActorContext& context)
    : Actor{context},
      _totalInserts{context.operation("Insert", HttpPoller::id())},
      ...
      //
      _loop{context, (*_client)[context["Database"].to<std::string>()], HttpPoller::id()} {}
